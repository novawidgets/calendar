...
(function() {
    ...
    var Calendar = Widget.extend({
        attrs: {
            /**
             * enable select while return true, similar to jquery.ui.datepicker
             */
        ...
        refresh: function() {
            ...
            $('.ui-calendar-calendar td:not(.ui-state-disabled), .ui-calendar-header a', el);
            $('.ui-calendar-header select', el).off('change', eventHandler);
            el.empty().append(this._generateHTML());

            highlight($('.ui-calendar-calendar td:not(.ui-state-disabled), .ui-calendar-header a', el), 'ui-state-hover');
        ...
        destroy: function() {
            ...
            $('.ui-calendar-calendar td:not(.ui-state-disabled)', el);
            $('.ui-calendar-header select', el).off('change', eventHandler);
            el.remove();
        ...
        _generateHTML: function() {
            ...
            for (i = 0; i < rows; i++) {
                html += '<tr>';
                for (j = 0; j < 7; j++) {
                    html += this._renderDay(j, printDate, firstDay, drawMonth, selectedDate, today, minDate, maxDate, opts.beforeShowDay);
        ...
        _renderDay: function(j, printDate, firstDay, drawMonth, selectedDate, today, minDate, maxDate, beforeShowDay) {
